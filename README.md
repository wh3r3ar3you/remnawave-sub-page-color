...
- Favicon из branding logo в панели (PNG/ICO/SVG) или переопределение через `BRANDING_ICON_URL`
...
```bash
git clone https://github.com/wh3r3ar3you/remnawave-sub-page-color.git
```

Перейдите в папку проекта:

```bash
cd remnawave-sub-page-color
```
...
Если запускаете на сервере за reverse proxy, оставьте привязку к `127.0.0.1`. Если страницу нужно открыть напрямую наружу, измените порт в `docker-compose.yml`, как описано в разделе "Порт".
...
По умолчанию favicon берётся из **branding → logo** в конфиге sub page в панели Remnawave. При старте контейнер сам скачивает logo, обрезает прозрачные поля и генерирует локальные PNG (`32/48/64/128/180`) с режимом `cover`, чтобы щит занимал почти весь квадрат во вкладке.

Подойдут `png`, `ico` и `svg`.

Чтобы задать свою иконку вместо logo из панели:
